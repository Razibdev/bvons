...
@section('content')
    <!-- Page Content -->
    <div class="content">
        <h2 class="content-heading">User Withdrawal</h2>
                ...
                <table class="table table-bordered table-light table-vcenter js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{$all_request->created_at->format('m/d/y h:i a')}}</td>
                            ...
                            <td class="text-center">
                                ...
                                {{--<div class="btn-group" role="group">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                        <form action="{{ route('user_verification.bp_request_accept', ["id" => $all_request->id]) }}" method="post">
                                            @csrf
                                            @method('patch')
                                            <button type="submit" class="dropdown-item text-success">
                                                <i class="fa fa-check mr-5"></i>Accepted
                                            </button>
                                        </form>
                                        <form action="{{ route('user_verification.bp_request_reject', ["user_verification" => $all_request->id]) }}" method="post">
                                            @csrf
                                            @method('patch')
                                            <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Are you sure want to do this?')">
                                                <i class="fa fa-remove mr-5"></i>Rejected
                                            </button>
                                        </form>
                                    </div>
                                </div>--}}
