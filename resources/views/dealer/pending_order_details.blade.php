                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach ($order->manyOrders as $order_detail)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">{{$order_detail->products->product_quantity}}</td>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="bg-white p-10">
                    ...
                    <form action="{{ route('vendor.orders.admin.change-status', ["order_id" => $order->id, "status" => "processing"]) }}" method="post" class="d-inline m-2">
                        @csrf
                        @method('patch')
                        @if($values == 1)
                            <button
                    ...
                    <form action="{{ route('vendor.orders.admin.change-status', ["order_id" => $order->id, "status" => "shipped"]) }}" method="post" class="d-inline m-2">
                        @csrf
                        @method('patch')
                        @if($values == 1)
                            <button
                    ...
                    <form action="{{ route('vendor.orders.admin.complete', ["order_id" => $order->id]) }}" method="post" class="d-inline m-2">
                        @csrf
                    ...
                    <i class="fa fa-angle-right"></i>
                    <form action="{{ route('vendor.orders.admin.change-status', ["order_id" => $order->id, "status" => "cancel"]) }}" method="post" class="d-inline m-2">
                        @csrf
                        @method('patch')
                        <button
