...
async function getDateRangeReportData(dys, start_date = null, end_date = null) {
    let url = window.orderDateRangeReportUrl;
    let urlQueryStirng = '';
    if(start_date !== null && end_date !== null) {
        urlQueryStirng = `?start_date=${start_date}&end_date=${end_date}`;
    }
    let newUrl = url + "" + urlQueryStirng;
    let dateRangeReport = await axios.get(newUrl);
    let dateRangeData = {
        pending : {},
        complete : {},
        cancel : {},
    ...
    for (let i = 0; i < dys.length ; i++) {
        dateRangeData["pending"][dys[i]] = dateRangeReport.data["pending"][dys[i]] ?? 0;
        dateRangeData["complete"][dys[i]] = dateRangeReport.data["complete"][dys[i]] ?? 0;
        dateRangeData["cancel"][dys[i]] = dateRangeReport.data["cancel"][dys[i]] ?? 0;
    }
...
class BeCompCharts {
     ...
     *
     */
    static async initChartsChartJS() {
        ...
        let days = getDates((new Date()).subDays(30) , new Date());
        let dys = days.map((d) => { return `${d.toISOString().slice(0,10)}`;});
        let dateRangeData = await getDateRangeReportData(dys);



        // Lines/Bar/Radar Chart Data
        let chartLinesBarsRadarData = {
            labels: [...Object.keys(dateRangeData['cancel'])],
            datasets: [
                {
                    label: 'Pending',
                    fill: true,
                    backgroundColor: 'rgba(63, 232, 255, .25)',
                    borderColor: 'rgb(63, 232, 255)',
                    pointBackgroundColor: 'rgba(63, 232, 255,1)',
                    pointBorderColor: '#fff',
                    pointHoverBackgroundColor: '#fff',
                    pointHoverBorderColor: 'rgba(63, 232, 255,1)',
                    data: Object.values(dateRangeData['pending'])
                },
                {
                    label: 'Complete',
                    fill: true,
                    backgroundColor: 'rgba(119, 199, 107, .25)',
                    borderColor: 'rgba(119, 199, 107,1)',
                    pointBackgroundColor: 'rgba(119, 199, 107,1)',
                    pointBorderColor: '#fff',
                    pointHoverBackgroundColor: '#fff',
                    pointHoverBorderColor: 'rgba(119, 199, 107,1)',
                    data: Object.values(dateRangeData['complete'])
                },
                {
                    label: 'Cancel',
                    fill: true,
                    backgroundColor: 'rgba(199, 107, 107, .25)',
                    borderColor: 'rgba(199, 107, 107,1)',
                    pointBackgroundColor: 'rgba(199, 107, 107,1)',
                    pointBorderColor: '#fff',
                    pointHoverBackgroundColor: '#fff',
                    pointHoverBorderColor: 'rgba(199, 107, 107,1)',
                    data: Object.values(dateRangeData['cancel'])
                }
            ]
        };
        ...
        // weekly report after clicking to weekly button
        $("#weekly-report").on('click', async function (){
            let res = await axios.get(window.orderWeeklyReportUrl);
            chartLinesBarsRadarData.labels = Object.keys(res.data['cancel']);
            chartLinesBarsRadarData.datasets[0].data = Object.values(res.data['pending']);
            chartLinesBarsRadarData.datasets[1].data = Object.values(res.data['complete']);
            chartLinesBarsRadarData.datasets[2].data = Object.values(res.data['cancel']);

            chartLines.update(chartLinesCon, { type: 'line', data: chartLinesBarsRadarData });

            $(this).addClass('active');
            $(".js-flatpickr").val('').removeClass('ku-active');
            if(window.fl) {
               fl.clear()
            }
            $("#monthly-report").removeClass('active')
        });

        // monthly report after clicking to monthly button
        $("#monthly-report").on('click',async function(){
            let days = getDates((new Date()).subDays(30) , new Date());
            let dys = days.map((d) => { return `${d.toISOString().slice(0,10)}`;});
            let dateRangeData = await getDateRangeReportData(dys);

            chartLinesBarsRadarData.labels = Object.keys(dateRangeData['cancel']);
            chartLinesBarsRadarData.datasets[0].data = Object.values(dateRangeData['pending']);
            chartLinesBarsRadarData.datasets[1].data = Object.values(dateRangeData['complete']);
            chartLinesBarsRadarData.datasets[2].data = Object.values(dateRangeData['cancel']);
            ...
            $("#weekly-report").removeClass('active');
        });

        // date range report after select the date range
        window.fl = $(".js-flatpickr").flatpickr({
            minDate: '2015-01-01',
            onClose: async function(selectedDates, dateStr, instance)  {
                let days = getDates(selectedDates[0] , selectedDates[1]);
                dys = days.map((d) => {
                    return `${d.toISOString().slice(0,10)}`;
                });

                let dateRangeData = await getDateRangeReportData(dys, selectedDates[0], selectedDates[1]);
                ...
                chartLines.update(chartLinesCon, { type: 'line', data: chartLinesBarsRadarData });
                $(".js-flatpickr").addClass('ku-active');
                $("#monthly-report").removeClass('active');
                $("#weekly-report").removeClass('active');
            },
