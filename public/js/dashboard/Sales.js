...
class Purchase extends Product {
        ...
        (function(){
            purchaseThis.purchaseItems.onkeyup = function(e) {
                ...
                if(e.target.classList.contains('purchase_quantity')) {
                    let purchaseQuantity = parseFloat(e.target.value);
                    let productId = e.target.getAttribute('product-id');
                    let productTotalQuantity = e.target.getAttribute('product-total-quantity');
                    let isPurchaseQuantityValid = true;

                    console.log(purchaseQuantity);

                    // validate the purchase quantity
                    if (purchaseQuantity === 0 || isNaN(purchaseQuantity)) {
                        e.target.classList.add('is-invalid');
                    ...
                    if(isPurchaseQuantityValid) {
                        if(purchaseQuantity > productTotalQuantity) {
                            e.target.classList.add('is-invalid');
                            isPurchaseQuantityValid = false;
                        } else {
                            e.target.classList.remove('is-invalid');
    ...
    updateTheProductPurchaseListGui(productDetails) {
        ...
        tr.innerHTML = `
            ...
            <td><input type="text" class="form-control p-1 border-radius-0 font-size-10 purchase_price" value="${productDetails['selling_price']}" product-id="${productDetails.id}"></td>
            <td><input type="text" class="form-control p-1 border-radius-0 font-size-10 purchase_quantity" value="" product-id="${productDetails.id}" product-total-quantity="${productDetails['product_total_quantity']}" placeholder="max:${productDetails['product_total_quantity']}" title="maximum: ${productDetails['product_total_quantity']}"></td>
